...
export async function POST(request: Request) {
  const rt = getRuntime();
  if (!rt.admin || process.env.LEASH_ADMIN_ENABLED !== "true") return NextResponse.json({ error: { code: "ADMIN_DISABLED", message: "server-side role writes are disabled" } }, { status: 403 });
  ...
  if (!account || (typeof body.roleBitmap !== "string" && typeof body.roleBitmap !== "number")) return NextResponse.json({ error: { code: "INVALID_INPUT", message: "account and roleBitmap are required" } }, { status: 400 });
  ...
  const hash = await rt.admin.grantRoles(rt.gate.resource, roleBitmap, account);
